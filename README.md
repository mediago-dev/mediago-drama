...
内置 Skills 位于 `packages/instructions/pkg/pack/builtin/assets/skills/`，服务端会通过内置 Prompt Pack 作为默认能力加载。
...
| `novel-writer` | 小说写作 | 在资料文档中撰写、续写、改写和润色小说正文，保持叙事视角、情节连续性和章节节奏 |

## 提示词模板与提示词库

项目内置多类提示词资产，覆盖 Agent 规范、工具使用、文档格式、角色图、场景图和视频镜头生成。Agent 指令由官方模板单独管理，提示词包只分发 Skills 与可复用提示词；文档格式模板由代码内置并通过 Skill 的 `template_id` 只读引用。

| 类型 | 路径 | 内容 |
| --- | --- | --- |
| Agent 指令 | `packages/instructions/pkg/official/assets/instructions/AGENTS.md` | 创作工作区中的 Agent 行为约束，可在设置中覆盖 |
| 工具指令 | `packages/instructions/pkg/official/assets/instructions/TOOLS.md` | 长文档写入、分批编辑、文档分类等工具调用规则，可在设置中覆盖 |
| 文档结构规则 | `packages/instructions/pkg/templates/assets/document-templates/` | 剧本、角色、场景、道具、分镜、小说资料等内置结构约束，由 `load_skill` 自动追加给 Agent，不作为 Skill 正文暴露 |
| 提示词库 | `packages/instructions/pkg/pack/builtin/assets/prompts/` | 角色概念图、多视图、场景氛围图、四视图、电影感镜头等 |
| 风格预设 | `packages/instructions/pkg/pack/builtin/assets/prompts/` | 真人写实、2D 动漫、3DCG 动漫、Q 版风格 |

## 技术架构

```text
apps/workspace/      React + Electron 创作工作台
services/server/     Go API 服务、Agent 编排、内置 Skills 与提示词模板
...
pnpm dev:desktop      # 仅启动 Electron 桌面端，需另开 dev:server
pnpm build            # 构建完整项目
