...
func TestACPClientPromptIdleRetryableAfterReadOnlyExecute(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	...
	err := client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.StartToolCall(
			"call-ls",
			"查看 work 目录结构",
			acp.WithStartKind(acp.ToolKind("execute")),
	...
	err = client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.UpdateToolCall(
			"call-ls",
			acp.WithUpdateTitle("查看 work 目录结构"),
			acp.WithUpdateKind(acp.ToolKind("execute")),
			acp.WithUpdateStatus(acp.ToolCallStatusInProgress),
			acp.WithUpdateRawInput(map[string]any{"command": `ls -la work/ 2>/dev/null || echo "work目录不存在"`}),
		),
	})
	if err != nil {
		t.Fatalf("SessionUpdate update returned error: %v", err)
	}
	client.mu.Lock()
	client.promptLastUpdateAt = time.Now().Add(-2 * time.Second)
	client.mu.Unlock()

	snapshot, idle := client.promptIdleSnapshot(time.Second)
	if !idle {
		t.Fatal("idle = false, want idle snapshot")
	}
	if !snapshot.Retryable || snapshot.MutatingToolCalls != 0 {
		t.Fatalf("snapshot = %#v, want retryable read-only execute", snapshot)
	}
}

func TestACPClientPromptIdleNotRetryableAfterMutatingExecuteUpdate(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	client.beginPromptMetrics()
	client.setAcceptingSessionUpdates(true)
	...
	err = client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.UpdateToolCall(
			"call-touch",
			acp.WithUpdateTitle("创建文件"),
			acp.WithUpdateKind(acp.ToolKind("execute")),
			acp.WithUpdateStatus(acp.ToolCallStatusInProgress),
			acp.WithUpdateRawInput(map[string]any{"command": `touch storyboard.md`}),
		),
	})
	if err != nil {
		t.Fatalf("SessionUpdate update returned error: %v", err)
	...
	if snapshot.Retryable || snapshot.MutatingToolCalls != 1 {
		t.Fatalf("snapshot = %#v, want non-retryable mutating execute", snapshot)
	}
}

func TestACPClientPromptIdleRetryableAfterPendingEditWithoutInput(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	client.beginPromptMetrics()
	client.setAcceptingSessionUpdates(true)
	...
	if !snapshot.Retryable || snapshot.MutatingToolCalls != 0 {
		t.Fatalf("snapshot = %#v, want retryable pending edit without input", snapshot)
	}
}

func TestACPClientPromptIdleRetryableAfterSkillNameContainingWrite(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	client.beginPromptMetrics()
	client.setAcceptingSessionUpdates(true)
	...
	err := client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.StartToolCall(
			"call-skill",
			"mediago_drama_load_skill",
			acp.WithStartKind(acp.ToolKindOther),
			acp.WithStartRawInput(map[string]any{"skill": "storyboard-writer"}),
		),
	})
	if err != nil {
		t.Fatalf("SessionUpdate skill start returned error: %v", err)
	}
	err = client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.UpdateToolCall(
			"call-skill",
			acp.WithUpdateTitle("storyboard-writer"),
			acp.WithUpdateKind(acp.ToolKindOther),
			acp.WithUpdateStatus(acp.ToolCallStatusCompleted),
			acp.WithUpdateRawInput(map[string]any{"skill": "storyboard-writer"}),
		),
	})
	if err != nil {
		t.Fatalf("SessionUpdate skill update returned error: %v", err)
	}
	err = client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.StartToolCall(
			"call-write",
	...
	if err != nil {
		t.Fatalf("SessionUpdate write start returned error: %v", err)
	}
	client.mu.Lock()
	client.promptLastUpdateAt = time.Now().Add(-2 * time.Second)
	client.mu.Unlock()
	...
	if !snapshot.Retryable || snapshot.MutatingToolCalls != 0 {
		t.Fatalf("snapshot = %#v, want retryable after loading storyboard-writer and pending write", snapshot)
	}
}

func TestACPClientPromptIdleRetryableAfterTodoWrite(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	client.beginPromptMetrics()
	client.setAcceptingSessionUpdates(true)
	...
	err = client.SessionUpdate(context.Background(), acp.SessionNotification{
		Update: acp.UpdateToolCall(
			"call-todos",
			acp.WithUpdateTitle("2 todos"),
			acp.WithUpdateKind(acp.ToolKindOther),
			acp.WithUpdateStatus(acp.ToolCallStatusCompleted),
			acp.WithUpdateRawInput(map[string]any{
				"todos": []map[string]string{
					{"content": "撰写分镜脚本", "status": "in_progress"},
					{"content": "校验字段", "status": "pending"},
				},
			}),
		),
	})
	...
	if !snapshot.Retryable || snapshot.MutatingToolCalls != 0 {
		t.Fatalf("snapshot = %#v, want retryable todowrite", snapshot)
	}
}

func TestACPClientPromptIdleNotRetryableAfterEditTool(t *testing.T) {
	client := &acpClient{publish: func(agentEvent) {}}
	client.beginPromptMetrics()
	client.setAcceptingSessionUpdates(true)
	...
	if snapshot.Retryable || snapshot.MutatingToolCalls != 1 {
		t.Fatalf("snapshot = %#v, want non-retryable edit tool", snapshot)
	}
}
