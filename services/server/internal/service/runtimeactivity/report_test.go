...
func TestSourcesReport(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "provider request in flight", inFlight: 1, expectBusy: true, expectedInFlight: 1},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			sources := Sources{
				...
				InFlightGenerationRequests: func() int64 { return testCase.inFlight },
			...
			if testCase.tasksErr == nil {
				...
				if report.InFlightGenerationRequests != testCase.expectedInFlight {
					t.Fatalf("InFlightGenerationRequests = %d, want %d", report.InFlightGenerationRequests, testCase.expectedInFlight)
				}
