...
- 需要形象一致或用户提到参考图时，在参数表单加 type=images 的参考图字段（default 预填已定稿图资产 id），提交的 id 数组填入 generate_media 的 referenceAssetIds。
- 优化提示词在参数表单用 type=prompt_optimization 字段（开关+文本模型+提示词包），不要用裸 toggle；提交 enabled 时把 routeId/referenceName/referencePrompt 填入 generate_media 的 promptOptimization。
- 为项目资源生成时建议带 notificationTarget（指向目标文档章节）：任务在回合结束后才完成时，用户会收到可点击跳转的生成完成通知。
