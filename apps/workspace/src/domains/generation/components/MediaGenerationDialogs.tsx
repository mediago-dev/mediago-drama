...
export const MaterialLibraryImportDialog: React.FC<{
	confirming?: boolean;
	...
	const availableMediaAssets = useMemo(
		() => mergeMaterialImportAssets(mediaAssets, uploadedAssets),
	...
	const draftSelectedAssetIdSet = useMemo(
		() => new Set(draftSelectedAssetIds),
		[draftSelectedAssetIds],
	);
	...
	const toggleDraftSelection = (asset: MediaAsset) => {
		setDraftSelectedAssetIds((current) =>
	...
	const uploadSelectedFiles = async (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			if (uploaded.length === 0) {
				setUploadError("上传完成，但没有可选择的图片素材。");
				return;
			}
			setUploadedAssets((current) => mergeMaterialImportAssets(current, uploaded));
			setDraftSelectedAssetIds((current) =>
				uniqueMaterialAssetIds([...current, ...uploaded.map((asset) => asset.id)]),
			);
		} catch (error) {
			setUploadError(materialImportErrorMessage(error, "图片上传失败。"));
		} finally {
			setIsUploading(false);
		}
	};
	...
	return (
		...
		>
			<section
				...
				role="dialog"
				aria-modal="true"
				aria-labelledby="generation-material-import-title"
				className="flex max-h-[min(42rem,calc(100vh-2rem))] w-full max-w-4xl flex-col overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-2xl data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0 duration-200"
			>
				<header className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-3">
					<div className="min-w-0">
						<h3
							id="generation-material-import-title"
							className="truncate text-sm font-semibold text-foreground"
						>
							从素材库中选择
						</h3>
						<p className="mt-1 truncate text-xs text-muted-foreground">
							确认后加入当前生成记录，再从生成记录中选择是否放入文档。
						</p>
					</div>
				...
				{uploadError ? (
					<p className="border-b border-border px-4 py-2 text-xs text-destructive">{uploadError}</p>
				) : null}
						...
						<div className="grid grid-cols-[repeat(auto-fill,minmax(9rem,1fr))] gap-3">
							{imageAssets.map((asset) => (
								<MaterialLibraryImportCard
									...
									onSelect={() => toggleDraftSelection(asset)}
								/>
							))}
						</div>
					)}
