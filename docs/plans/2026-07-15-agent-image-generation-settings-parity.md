...
- 用户可维护技能包里的“风格”分类继续保留；用户新增或删除技能包后，表单实时同步。
...
- 图片 Skill 明确要求直接打开统一生成设置表单，风格来自用户可维护技能包或业务 prompt。
...
- 更新 MCP instructions 和 `ListModels` 描述，说明风格/补充内容由统一表单中的动态技能包选择。
...
- 优化提示词关闭时规范化为 `{ enabled: false }`；开启时必须有 text route 和技能包。
- 用户已删除的技能包 ID 会从恢复状态中清理。
...
- batch preference 与完整提交值之间的 adapter；localStorage 只保存 route/params/技能包 ID/开关，不保存参考图。
...
- 同 kind 恢复 `generation.batch-settings.v1` 中最近一次模型、route 参数、补充技能包和优化配置。
- Agent 卡片传入的本次任务默认值覆盖已保存偏好；用户开始编辑后，catalog 刷新不能覆盖当前值。
- 用户删除技能包后，已保存但已失效的 ID 被清理；用户新增任意分类（包括名字叫“风格”的分类）后可立即选择。
- image route 不支持参考图时自动清空 references；支持时保留已有资产 ID。
- 补充提示词开启但未选包、优化开启但缺文本 route/技能包时，`isValid=false`。
...
再覆盖：动态 route 参数组合、补充提示词多选、优化提示词单选+文本模型、用户自定义“风格”技能包，以及每次编辑都向上游发出完整 `GenerationSettingsValue`。
...
- 卡片上没有独立 `stylePresets`/通用“视觉风格”控件，但动态“风格”技能包和 route 的 `style` 参数仍正常显示。
...
- 新增一个任意分类技能包后立刻可见；删除当前已选包后状态被清理并阻止无效提交。
...
- 用户自定义技能包（包括名为“风格”的分类）仍可增删；route schema 的 `style` 字段仍可用。
