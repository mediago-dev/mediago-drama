...
MediaGo Drama 现有 `/api/v1/skills` 和 `SkillsEditorPanel` 管理的是技能包中的应用内 Skill，数据来自 PackStore/SQLite。它们不是 Codex 从本机文件系统发现的 Skill，因此不能扩展现有 `SkillMeta.source = pack | user` 来混装 Codex Skill，否则会混淆生命周期、权限和删除语义。
...
1. 标题“Codex 全局技能”，说明“只读展示 MediaGo 服务进程所在设备检测到的 Codex Skill；App/CLI 使用不同 CODEX_HOME 时结果可能不同；与技能包相互独立”。
